...
pub fn count<I: InsnSet>(stream: LDIter<I>, min_bytes: usize) -> Result<usize, usize> {
	let mut written = 0;
...
pub fn trampoline<I: InsnSet, W: io::Write>(stream: LDIter<I>, buf: &mut W, min_bytes: usize) -> io::Result<()> {
	let mut written = 0;
	let stream = stream.take_while(|&(opcode, _)| { written += opcode.len(); written < min_bytes });

	for (opcode, va) in stream {
...
fn main() {
	assert_eq!(count(x86::iter(INPUT_CODE, 0x1000), 5), Ok(9));
}
